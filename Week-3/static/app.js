...
let liveActual = [], livePred = [], liveTimes = [], MAX = 120;
...
socket.on('live_data', (d) => {
    liveTimes.push(d.timestamp); liveActual.push(d.actual); livePred.push(d.predicted);
    if (liveTimes.length > MAX) { liveTimes.shift(); liveActual.shift(); livePred.shift(); }
    Plotly.react('liveChart', [
        { x: liveTimes, y: liveActual, type: 'scatter', mode: 'lines', name: 'Actual', line: { color: 'rgb(52,211,153)', width: 2 } },
        { x: liveTimes, y: livePred, type: 'scatter', mode: 'lines', name: 'Predicted', line: { color: 'rgb(94,234,212)', width: 2, dash: 'dot' } }
...
function uKPI(d) {
    ...
    <div class="card kpi"><div class="ki"><i class="fa-solid fa-arrow-up"></i></div><div class="kv">+${s.surge_delta}</div><div class="kl">Surge Delta (kWh)</div></div>`;
}

function cSurge(data) {
    const lunch = data.filter(d => d.hour >= 12 && d.hour <= 14 && !d.is_weekend);
    const other = data.filter(d => !(d.hour >= 12 && d.hour <= 14) || d.is_weekend);
    Plotly.newPlot('surgeChart', [
        { y: lunch.map(d => d.energy_kwh), type: 'box', name: 'Lunch (12-14h)', marker: { color: 'rgb(52,211,153)' } },
        { y: other.map(d => d.energy_kwh), type: 'box', name: 'Other Hours', marker: { color: 'rgba(52,211,153,.3)' } }
    ], { ...L, showlegend: false }, C);
}

function cCoef(d) {
    if (!d || !d.metrics) return; const c = d.metrics.coefficients; const keys = Object.keys(c), vals = keys.map(k => c[k]);
    const colors = vals.map(v => v >= 0 ? 'rgb(52,211,153)' : 'rgb(239,68,68)');
    Plotly.newPlot('coefChart', [{ x: vals, y: keys, type: 'bar', orientation: 'h', marker: { color: colors } }], { ...L, margin: { ...L.margin, l: 120 }, yaxis: { ...L.yaxis } }, C);
}

function cTemp(data) { Plotly.newPlot('tempChart', [{ x: data.map(d => d.temperature_c), y: data.map(d => d.energy_kwh), type: 'scatter', mode: 'markers', marker: { size: 3, color: 'rgba(52,211,153,.3)', line: { width: 0 } } }], { ...L, xaxis: { ...L.xaxis, title: 'Temperature (°C)' }, yaxis: { ...L.yaxis, title: 'Energy (kWh)' } }, C); }
...
function cHourly(data) {
    ...
    const colors = hrs.map(i => (i >= 12 && i <= 14) ? 'rgb(52,211,153)' : 'rgba(52,211,153,.35)');
    Plotly.newPlot('hourlyChart', [{ x: hrs, y: vals, type: 'bar', marker: { color: colors } }], { ...L, xaxis: { ...L.xaxis, title: 'Hour', tickmode: 'linear', dtick: 2 }, yaxis: { ...L.yaxis, title: 'Avg kWh' } }, C);
}

async function load() {
    const [st, hist] = await Promise.all([f('stats'), f('historical')]);
    if (st) { uKPI(st); cCoef(st); }
    if (hist && hist.data) { cSurge(hist.data); cTemp(hist.data); cHourly(hist.data); }
}
